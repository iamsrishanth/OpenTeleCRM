...
const AGENT_NAV = [
  { href: '/agent', label: 'Agent Calling Hub', icon: Headphones, badge: 'CALLER' },
...
const NAV = [
  ...
  { href: '/dialer', label: 'Dialer Session', icon: PhoneCall },
...
export function Sidebar() {
  ...
  return (
    <aside className="flex w-60 shrink-0 flex-col border-r border-border bg-sidebar">
      <Link href="/agent" className="flex h-14 shrink-0 items-center gap-2.5 border-b border-sidebar-border px-4 hover:bg-sidebar-accent/50 transition-colors">
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto p-3">
        {/* Agent Telecaller Section */}
        <div>
          <p className="px-3 pb-1.5 text-[11px] font-bold uppercase tracking-wider text-primary">
            Telecaller Desk
          </p>
          {AGENT_NAV.map(({ href, label, icon: Icon, badge }) => {
            ...
            return (
              ...
              >
                ...
                {badge && (
                  <span className="text-[9px] font-extrabold bg-primary/20 text-primary px-1.5 py-0.5 rounded">
                    {badge}
                  </span>
                )}
              </Link>
