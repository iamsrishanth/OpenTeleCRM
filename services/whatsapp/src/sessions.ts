...
export type WhatsAppDriver = 'mock' | 'wwebjs' | 'baileys' | 'bridge' | 'cloud-api';
...
export async function providerFor(
  ...
  if (kind === 'wwebjs') {
    ...
    const { BridgeProvider } = await import('./providers/bridge.provider.js');
...
export function resolveWhatsappDriver(): WhatsAppDriver {
  ...
  return d === 'wwebjs' || d === 'baileys' || d === 'bridge' ? d : 'mock';
}
