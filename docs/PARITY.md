...
|---|---|---|---|---|---|---|
| F1 | Multi-tenant foundation: every table enterprise-scoped, RLS enabled + FORCEd on all 28 tenant tables, `app.enterprise_id` session-variable policy, `withTenant()` txn wrapper | `packages/db` (`schema.ts`, `whatsapp-schema.ts`, `telephony-schema.ts`, `automation-schema.ts`, `rls.ts`, migrations `drizzle/0000…0007`) | Drizzle ORM, node-postgres, PostgreSQL 16/17 | ✅ | — | `services/api/src/__tests__/metadata.contract.test.ts` (all suites run through RLS); no dedicated RLS unit test yet |
...
| TeleCRM ID | Feature | Our module | OSS deps | Status | Divergence note | Test IDs |
|---|---|---|---|---|---|---|
| A2.1 | 1-Click WhatsApp | `services/whatsapp` mock + **baileys** + **bridge** drivers (`sendText`), API `POST /enterprise/{eid}/whatsapp/send` | Baileys (live, pairing-code) / standalone whatsapp-bridge (deploy-anywhere), contracts | ✅ | API + mock driver green; **live outbound verified end-to-end** via the standalone `bridge` driver (`services/whatsapp-bridge` — Baileys 7.x, which pairs business/smba numbers that 6.x rejects with 401). Direct Baileys pairing remains for in-process use (`pair -- --code <phone>`); the bridge adds a deploy-anywhere HTTP surface + owned inbound queue (driver polls it for chat sync) | `whatsapp-inbox.contract.test.ts` + `bridge.test.ts` (8 tests) |
...
| A3.2 | Inbound capture API / webhooks | `services/api` (`automation/webhook.controller.ts` — HMAC-authenticated `POST /webhook/:tenantId/:name` — `X-OT-Signature: sha256=<hex>` over `tenantId\nname\nJSON.stringify(body)`; secret issued at rule create/rotate, never in list/get — fires matching `webhook_received` automation, writes `automation_run` rows) | Drizzle, NestJS | ✅ | Authorization is the HMAC signature, not the slug (fail-closed: missing/bad signature or secret-less rule → 401); generic capture-connector layer (P5 lead capture) not yet built | `automation.contract.test.ts` (webhook inbound suite) |
...
| A4.7 | Automation quota metering | `services/api` (`automation/meter.ts` — per-tenant sliding-window rate limiter; `automation_quota` table for per-enterprise overrides; `GET /automations/usage`, `GET|PUT /automations/quota`) | Drizzle, NestJS | ✅ | D4 fix: unlimited automations + documented, observable per-tenant limiter (runs/min) — over-limit firings record a visible `throttled` run instead of an opaque TeleCRM-style quota | `automation.quota.contract.test.ts` (5 suites) |
...
## A9 — Workforce Management (ByteCodeEMS port)

| TeleCRM ID | Feature | Our module | OSS deps | Status | Divergence note | Test IDs |
|---|---|---|---|---|---|---|
| A9.1 | Attendance (GPS check-in/out, late/half-day rules) | `packages/db` (`attendance`), `services/api` (`workforce/attendance.controller.ts`), `apps/web/attendance`, `apps/mobile` `:feature:attendance` | Drizzle, NestJS, LocationManager | ✅ | ≤09:30 → present, >09:30 → late, <4h → half_day; one row per member per work_date | `workforce.contract.test.ts` |
| A9.2 | EOD reports (6 PM cutoff) | `packages/db` (`eod_report`), `services/api` (`workforce/eod.controller.ts`), web + mobile | Drizzle, NestJS | ✅ | Late ≥18:00; `processEodCutoff` marks non-submitters missed; late submission after cutoff upgrades missed→late | `workforce.contract.test.ts` + jobs suite |
| A9.3 | Task assignment & status flow | `packages/db` (`task`), `services/api` (`workforce/tasks.controller.ts`), web + mobile | Drizzle, NestJS | ✅ | todo→in_progress→blocked→done; completed_at on done; `task_assigned` event | `workforce.contract.test.ts` |
| A9.4 | Departments & heads | `packages/db` (`department`), `services/api` (`workforce/departments.controller.ts`), web admin | Drizzle, NestJS | ✅ | head_member_id → team_member; admin-gated CRUD | `workforce.contract.test.ts` |
| A9.5 | Configurable metrics vs targets | `packages/db` (`metric_definition`/`target`/`daily_metric_entry`), `services/api` (`workforce/metrics.controller.ts`) | Drizzle, NestJS | ✅ | Data-driven dept metrics (Sales = leads/calls); daily view vs default target | `workforce.contract.test.ts` |
| A9.6 | Weekly reports (Saturday rollup) | `packages/db` (`weekly_report`), `services/api` (`workforce/reports.controller.ts` + `system-jobs.ts`) | Drizzle, NestJS | ✅ | Mon–Sat week; `processWeeklyRollup` computes days_present/eod/tasks/metric totals | jobs suite |
| A9.7 | CSV exports (EOD/attendance/weekly) | `services/api` (`workforce/reports.controller.ts` export routes) | NestJS | ✅ | Admin-only; text/csv via passthrough response | `workforce.contract.test.ts` |
| A9.8 | Device call tracking (per-SIM) | `packages/db` (`device_call`), `services/api` (`workforce/device-calls.controller.ts`), `apps/mobile` `:feature:calls` | Drizzle, NestJS, CallLog | ✅ | Batched import ≤500; SIM detection via PHONE_ACCOUNT_ID; Room cache + sync | `workforce.contract.test.ts` |
| A9.9 | Team administration (role/dept/status) | `services/api` (`workforce/team-admin.controller.ts`, `me.controller.ts`), web admin | Drizzle, NestJS | ✅ | GET/PATCH `/team/:memberId` (admin); GET `/me` identity; distinct from TeleCRM `/team-members` sync surface | `workforce.contract.test.ts` |
| A9.10 | System jobs + automation events | `services/api` (`workforce/system-jobs.ts`, `workforce/events.ts`, `automation/scheduler.ts`) | NestJS | ✅ | EOD cutoff 12:30 UTC Mon–Sat, weekly rollup Sat, overdue-task sweep daily (UTC-shifted cron); 6 new trigger kinds | jobs suite |
...
| D4 | **Automation quota ambiguity** — TeleCRM plan docs state automation quotas inconsistently across surfaces; unclear what counts | We ship **unlimited automations** in community mode with a **per-tenant rate limiter** (documented, observable) instead of an opaque quota | Implemented (A4.7) |
...
| A2 WhatsApp | 2 (A2.1, A2.8 sequences) | 2 (A2.2, A2.4) | 4 (A2.3, A2.5, A2.6, A2.7) |
| A3 Lead Capture | 1 (A3.2 webhook) | 0 | 1 (A3.1) |
| A4 Automation | 7 (A4.1, A4.2, A4.3, A4.4, A4.5, A4.6, A4.7) | 0 | 0 |
| A5 Reports | 0 | 0 | 6 |
| A6 Customization & Admin | 9 (A6.1, A6.2, A6.5, A6.6, A6.6a, A6.6b, A6.6c, A6.6d, A6.7) | 2 (A6.3, A6.4) | 0 |
| A7 AI & Voice | 0 | 0 | 5 |
| A8 Support & Onboarding | 0 | 0 | 3 |
| A9 Workforce Mgmt (ByteCodeEMS port) | 10 (A9.1–A9.10) | 0 | 0 |
| B Plans & Billing | 0 | 0 | 3 |

**Implemented and verified:** multi-tenant foundation + RLS (F1, **37 tenant tables**, 10 migrations `0000–0009`), seed data (F2), TeleCRM-parity metadata REST surface (A6.6a), 13-tool MCP surface (A6.6b), full Sync API (A6.6c), full Async API (A6.6d with `?validate=true` dry-run — divergence D1), API tokens with class enforcement (A6.6, divergence D2), custom fields / pipeline-stage / workspace settings read paths (A6.1, A6.2, A6.5), audit-log write path (A6.7). **P2 WhatsApp (API-level ✅, live outbound):** contracts + provider abstraction, mock + wwebjs + baileys drivers, unified inbox with auto lead-attribution, templates CRUD, broadcasts (create/start/opt-out), sequences/drips (A2.8) — plus the standalone deploy-anywhere `services/whatsapp-bridge` (Baileys 7.x, own session + queue) driving live outbound — 22/22 whatsapp package unit tests (bridge 8 + mock-provider 8 + sessions 6) + inbox/template/broadcast/sequence contract suites. **P3 Telephony (✅, live dial):** `TelephonyProvider` contract + call domain types, telephony schema (`call`/`recording`/`callback`/`dnd_registry`, migration 0002, RLS-wired), mock + asterisk-ari providers + pure dialer scoring, API module (calls A1.3, caller-id A1.6, dialer A1.1, callbacks A1.5, recordings A1.2 partial) — 11/11 telephony unit tests; **P4b added live ARI dialing** (`POST /dialer/:leadId/dial` → Asterisk originate, Stasis event bridge, migration 0006 `provider_call_id`). **P4 Automation (✅):** rules engine (`packages/rule-engine` pure evaluator), `automation`/`automation_run`/`automation_step` schema (migration 0003), rules CRUD + `/:id/test`, 9 event kinds, **10 action executors** (assign_lead, create_callback, send_whatsapp, update_field, move_stage, notify_user + delay/branch/http_request/webhook; only `send_email` stubbed), lead distribution (round-robin/least_loaded/skill_match), HMAC-authenticated public webhook trigger (X-OT-Signature; fail-closed), 60s in-process scheduler, 10 seeded templates (A4.6), React Flow visual builder (A4.1), + A4.7 per-tenant quota metering (D4 fix, migration 0005) — **103/103 API contract tests** (13 files) + **15/15 MCP** + 11/11 telephony + 22/22 whatsapp. **Bruno collection (`collections/opentelecrm/`) runs green against the live API (27 requests, 26 assertions).** Real-number pairing is a documented CLI step (`pnpm --filter @opentelecrm/whatsapp pair`). **Mobile (P8 shipped ahead of roadmap as Kotlin native):** Compose app with leads/dialer/inbox/team/settings, Room offline cache + outbox, WorkManager flush, UnifiedPush, F-Droid metadata (M0–M5, `apps/mobile`). **W1 Workforce management (ByteCodeEMS port, ✅):** attendance/EOD/tasks/departments/metrics/weekly-reports/exports/device-calls (A9.1–A9.10) — 37 tenant tables, `workforce` API module (9 controllers), code-driven system jobs (EOD cutoff 12:30 UTC Mon–Sat, Saturday weekly rollup, daily overdue-task sweep) with 6 new automation trigger kinds, web pages + role-gated admin, Kotlin features (attendance GPS, eod, tasks, calls w/ SIM-aware CallLog import) — contract suite **119/119** (workforce file adds 12). Partial: roles enforcement (A6.3), team read/write admin UI (A6.4), WhatsApp cloud-api/chatbot/widget/notifications (A2.3/A2.5/A2.6/A2.7), recording pipeline (A1.2 full), enterprise auth via Zitadel OIDC (future). Everything in A1.4, A1.7, A1.8, A3.1, A5, A7, A8, B is not yet built.

_Last updated: 2026-08-10. Keep in sync with `services/api`, `services/mcp`, `packages/db`, `apps/mobile` as features land._
